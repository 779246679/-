    ...
    >
      ...
      <div style="width: 320px; margin-top: 10px">
        <div
          ...
          class="clearfix"
        >
          <div v-if="isReduce" style="width: 15%">
            <i class="el-icon-d-arrow-left" style="opacity: 0.3"></i>
          </div>
          <div v-else aria-label="前一年" style="width: 15%" @click="prev">
            <i class="el-icon-d-arrow-left"></i>
          </div>
          <span style="width: 70%">{{ year }}</span>
          <div v-if="isAdd" style="width: 15%">
            <i class="el-icon-d-arrow-right" style="opacity: 0.3"></i>
          </div>
          <div v-else aria-label="后一年" style="width: 15%" @click="next">
        ...
        <div class="textitem">
          ...
          >
            <el-radio
              ...
              :key="index"
              :label="`${DateList[curIndex].TimeYear}-Q${item}`"
              class="oneitem QuarterlySelect"
            >
              {{
...
export default {
  ...
  data() {
    return {
      ...
      curIndex: 0,
      quarterName: [
        { id: 1, name: "第1季度", name_en: "Q1th", data: "01" },
        { id: 2, name: "第2季度", name_en: "Q2nd", data: "04" },
        { id: 3, name: "第3季度", name_en: "Q3rd", data: "07" },
        { id: 4, name: "第4季度", name_en: "Q4th", data: "10" },
      ],
      isDisabled: Array.from({ length: 4 }).map(() => {
        return false;
      }),
  ...
  methods: {
    ...
    focusselect() {
      this.$nextTick(() => {
        ...
        if (this.isHasfilter) {
          for (const element of this.selectList) {
            for (let i = 0; i < lists.length; i++) {
              let text = this.changeQuarterFormat(lists[i].innerText);
              if (
                Number(element.start_time.slice(0, 4)) === Number(this.year) &&
                text === element.start_time.slice(5, 7)
              ) {
